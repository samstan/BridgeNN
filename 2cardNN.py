...
#Code for two card variation of double dummy bridge neural network
...
for i in range(0,100000):
  x_list.append(torch.load(os.path.join('trn2', str(i)+'.pt')))
...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, poolSize)
        x = self.fc1(x)
        return 2*torch.sigmoid(x) #since output is now between 0 and 1

model = Net()

loss_fn = torch.nn.MSELoss() #change to mean squared error, since binary cross entropy works only for normal sigmoid
...
for i in range(0,10000):
  x_list.append(torch.load(os.path.join('val2', str(i)+'.pt')))
...
#to count the 0, 1, and 2 loss (which will always by 100 in this case)
correct = 0
y0 = 0
offByOne = 0
y1 = 0
